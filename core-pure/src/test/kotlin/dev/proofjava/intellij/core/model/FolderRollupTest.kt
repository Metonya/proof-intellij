...
class FolderRollupTest {
    ...
    private fun fileWith(numerator: Long, denominator: Long): FileCoverageEntry {
        ...
        return FileCoverageEntry(module = "root", path = "x", metrics = MetricSet(MetricSet.JACOCO_LINE, metric, metric, metric), lines = emptyList())
    }

    @Test
    fun `sums numerators and denominators across files, one division`() {
        val rollup = rollupFolder(listOf(fileWith(8, 10), fileWith(2, 10)), BadgeMetric.ENGINE_LINE)
    ...
    fun `an empty file list has a null percent, not a divide-by-zero NaN`() {
        val rollup = rollupFolder(emptyList(), BadgeMetric.ENGINE_LINE)
